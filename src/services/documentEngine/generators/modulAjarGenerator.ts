...
export async function generateModulAjar(context: DocumentGenerationContext): Promise<GeneratedDocumentResult> {
  ...
  const tpList = tp?.items && tp.items.length > 0
    ? tp.items.map((t, idx) => `${idx + 1}. [${t.code}] ${t.statement}`).join('\n')
    : atp.items.map((a, idx) => `${idx + 1}. [${a.tpCode}] ${a.tpStatement}`).join('\n');
  ...
  addSubSection(
    ...
    `1. Peserta Didik Reguler/Tipikal: Umum, tidak ada kesulitan dalam mencerna dan memahami materi ajar.\n2. Peserta Didik dengan Kesulitan Belajar: Memiliki gaya belajar tertentu atau membutuhkan bimbingan bertahap (scaffolding).\n3. Peserta Didik dengan Pencapaian Tinggi: Mampu mencerna materi dengan cepat dan terampil memecahkan masalah tingkat tinggi (HOTS).`
  );
  addSubSection(
